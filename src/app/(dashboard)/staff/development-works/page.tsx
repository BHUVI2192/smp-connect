...
const WORK_STATUSES = ["PROPOSED", "APPROVED", "IN_PROGRESS", "COMPLETED", "DELAYED", "CANCELLED"];
...
const emptyForm = {
  title: "", sector: "ROADS", status: "PROPOSED",
  ...
  budget: "", budget_used: "", progress_pct: "0",
  location: "",
};
...
export default function StaffDevelopmentWorksPage() {
  ...
  async function fetchWorks() {
    setLoading(true);
    const res = await fetch("/api/development-works?pageSize=200", { credentials: "include" });
    if (res.status === 401) {
  ...
  async function handleSubmit() {
    ...
    if (!res.ok) {
      toast.error("Failed to create development work");
      setSubmitting(false);
      return;
    }
    ...
    toast.success("Development work created successfully!");
    setSubmitting(false);
  ...
  async function deleteWork() {
    ...
    if (res.ok) {
      toast.success("Development work deleted");
      setWorks((prev) => prev.filter((w) => w.id !== confirmDeleteId));
    } else {
      toast.error("Failed to delete development work");
    }
    setDeleting(false);
  ...
  function openEdit(work: any) {
    setEditForm({
      ...
      sector: work.sector || "ROADS",
      status: work.status || "PROPOSED",
      location: work.location || "",
      latitude: work.latitude ? String(work.latitude) : "",
      longitude: work.longitude ? String(work.longitude) : "",
      contractor: work.contractor || "",
      fundSource: work.fundSource || "",
      startDate: work.startDate ? work.startDate.slice(0, 10) : "",
      endDate: work.endDate ? work.endDate.slice(0, 10) : "",
      remarks: work.remarks || "",
      budget: work.budget ? String(work.budget) : "",
      budget_used: work.budget_used ? String(work.budget_used) : "",
      progress_pct: work.progress_pct ? String(work.progress_pct) : "0",
      stateId: work.stateId ? String(work.stateId) : "",
      districtId: work.districtId ? String(work.districtId) : "",
      talukId: work.talukId ? String(work.talukId) : "",
      panchayatId: work.panchayatId ? String(work.panchayatId) : "",
      villageId: work.villageId ? String(work.villageId) : "",
    });

    // Populate edit location dropdowns
    if (work.stateId) fetchLoc("districts", String(work.stateId), setEditDistricts);
    if (work.districtId) fetchLoc("taluks", String(work.districtId), setEditTaluks);
    if (work.talukId) fetchLoc("panchayats", String(work.talukId), setEditPanchayats);
    if (work.panchayatId) fetchLoc("villages", String(work.panchayatId), setEditVillages);
  ...
  async function saveEdit() {
    ...
    const res = await fetch("/api/development-works", {
      ...
      body: JSON.stringify({
        ...
        budget_used: editForm.budget_used ? parseFloat(editForm.budget_used) : null,
        progress_pct: editForm.progress_pct ? parseInt(editForm.progress_pct) : null,
        latitude: editForm.latitude ? parseFloat(editForm.latitude) : null,
        longitude: editForm.longitude ? parseFloat(editForm.longitude) : null,
        stateId: editForm.stateId ? parseInt(editForm.stateId) : null,
        districtId: editForm.districtId ? parseInt(editForm.districtId) : null,
        talukId: editForm.talukId ? parseInt(editForm.talukId) : null,
        panchayatId: editForm.panchayatId ? parseInt(editForm.panchayatId) : null,
        villageId: editForm.villageId ? parseInt(editForm.villageId) : null,
      }),
    });

    if (res.ok) {
      toast.success("Development work updated");
      setEditingWork(null);
      fetchWorks();
    } else {
      toast.error("Failed to update development work");
    }
    setEditSaving(false);
  }
  ...
  const totalBudgetUsed = works.reduce((s, w) => s + (w.budget_used || 0), 0);
  ...
  if (viewMode === "edit" && editingWork) {
    ...
    const existingMedia: any[] = editingWork?.media || [];

    return (
      <div className="space-y-6">
        {/* Header */}
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" onClick={() => { setViewMode("list"); setEditingWork(null); setEditStep(1); }}>
            <ArrowLeft className="h-5 w-5" />
          </Button>
          <div className="flex-1">
            <h1 className="text-xl font-bold text-gray-900">Edit Development Work</h1>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Select value={editForm.status} onValueChange={(v) => setEditForm({ ...editForm, status: v })}>
                    <SelectTrigger className="mt-1"><SelectValue /></SelectTrigger>
                    <SelectContent>{WORK_STATUSES.map((s) => <SelectItem key={s} value={s}>{s.replace(/_/g, " ")}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
              </div>
              <div>
                <Label>Fund Source</Label>
                <Input className="mt-1" value={editForm.fundSource} onChange={(e) => setEditForm({ ...editForm, fundSource: e.target.value })} placeholder="e.g. State Budget, Central Scheme, Local Fund" />
              </div>
              <div>
        ...
        {editStep === 2 && (
          <Card>
            <CardHeader>
              <CardTitle className="text-base flex items-center gap-2">
                <IndianRupee className="h-4 w-4 text-green-600" /> Financial Details & Progress
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                  <Label>Budget Used / Utilized (₹)</Label>
                  <div className="relative mt-1">
                    <IndianRupee className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-gray-400" />
                    <Input
                      type="number"
                ...
                <div>
                  ...
                  <Input
                    ...
                    max="100"
                    className="mt-1"
                    value={editForm.progress_pct || "0"}
                    onChange={(e) => setEditForm({ ...editForm, progress_pct: e.target.value })}
                    placeholder="0"
                  />
                </div>
              </div>
              ...
              {editForm.budget && (
                <div className="p-4 bg-gray-50 rounded-lg space-y-3">
                  <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">Project Financial Progress</p>
                  <div className="grid grid-cols-3 gap-4 text-center">
                    <div>
                      <p className="text-xs text-gray-400">Total Budget</p>
                      <p className="font-bold text-gray-900">{formatCurrency(parseFloat(editForm.budget) || 0)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-400">Utilized</p>
                      <p className="font-bold text-green-600">{formatCurrency(parseFloat(editForm.budget_used) || 0)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-400">Balance</p>
                      <p className="font-bold text-indigo-600">{formatCurrency((parseFloat(editForm.budget) || 0) - (parseFloat(editForm.budget_used) || 0))}</p>
                    </div>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div
                      className="h-2 rounded-full bg-blue-600 transition-all"
                      style={{ width: `${Math.min(parseInt(editForm.progress_pct) || 0, 100)}%` }}
                    />
                  </div>
                  <p className="text-center text-xs text-gray-500 font-semibold">
                    {editForm.progress_pct || 0}% Physically Completed
                  </p>
                </div>
              )}
          ...
          <Card>
            ...
            <CardContent className="space-y-5">
              <div className="space-y-1">
                <p className="text-xs font-semibold uppercase tracking-wider text-gray-400">Geographic Bounds</p>
                <Separator />
              </div>
              ...
              <div className="space-y-1 pt-2">
                <p className="text-xs font-semibold uppercase tracking-wider text-gray-400">Timelines</p>
                <Separator />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
              ...
              <div className="flex justify-between pt-2">
                ...
                <Button onClick={() => setEditStep(4)}>Next: Media & Review →</Button>
              </div>
            </CardContent>
          </Card>
        )}
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center justify-between">
                    ...
                    <Badge variant="outline">{existingMedia.length}</Badge>
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {(["BEFORE", "DURING", "AFTER"] as const).map((phase) => {
                    const pPhotos = existingMedia.filter((p) => p.phase === phase);
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <input
                  ...
                  multiple
                  className="hidden"
                  onChange={(e) => { if (e.target.files && editForm.id) uploadExistingMedia(e.target.files, editForm.id); e.target.value = ""; }}
                />
                <div
                  ...
                  onDrop={(e) => { e.preventDefault(); if (e.dataTransfer.files && editForm.id) uploadExistingMedia(e.dataTransfer.files, editForm.id); }}
                >
                  {existingUploading ? (
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <h1 className="text-xl font-bold text-gray-900">New Development Work</h1>
            <p className="text-sm text-gray-500">Record a new infrastructure project</p>
          </div>
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <Input
                  ...
                  placeholder="e.g. State Budget, Central Scheme, Local Fund"
                />
              </div>
              <div>
                <Label>Contractor / Implementing Agency</Label>
                ...
                <div>
                  ...
                  <div className="relative mt-1">
                    ...
                    <Input
                      ...
                      onChange={(e) => setForm({ ...form, budget_used: e.target.value })}
                      placeholder="0"
                    />
                  </div>
                </div>
              ...
              {form.budget && (
                <div className="p-4 bg-gray-50 rounded-lg space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-4 text-center">
                    <div>
                      <p className="text-xs text-gray-400">Total Budget</p>
                      <p className="font-bold text-gray-900">{formatCurrency(parseFloat(form.budget) || 0)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-400">Utilized</p>
                      <p className="font-bold text-green-600">{formatCurrency(parseFloat(form.budget_used) || 0)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-400">Balance</p>
                      <p className="font-bold text-indigo-600">{formatCurrency((parseFloat(form.budget) || 0) - (parseFloat(form.budget_used) || 0))}</p>
                    </div>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div
                      className="h-2 rounded-full bg-blue-600 transition-all"
                      style={{ width: `${Math.min(parseInt(form.progress_pct) || 0, 100)}%` }}
                    />
                  </div>
                  <p className="text-center text-xs text-gray-500 font-semibold">
                    {form.progress_pct || 0}% Physically Completed
                  </p>
                </div>
              )}
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                          {pPhotos.map((photo) => (
                            <div key={photo.id} className="group relative aspect-video bg-gray-100 rounded-lg overflow-hidden border border-gray-200">
                              ...
                              <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                <button
                                  className="p-1.5 bg-red-500/95 rounded-full hover:bg-red-600"
            ...
            <div className="flex items-center justify-between">
              ...
              <Button onClick={handleSubmit} disabled={submitting} size="lg" className="gap-2">
                {submitting ? (
                  ...
                  <><Check className="h-4 w-4" /> Create Development Work</>
                )}
              </Button>
            </div>
          </div>
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Development Works" description="Track and update general constituency infrastructure and development projects">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard title="Total Works"       value={works.length}                  icon={<BarChart3  className="h-5 w-5 text-blue-600"   />} />
        <StatCard title="Total Budget"      value={formatCurrency(totalBudget)}   icon={<IndianRupee className="h-5 w-5 text-indigo-600" />} />
        <StatCard title="Budget Utilized"  value={formatCurrency(totalBudgetUsed)} icon={<TrendingUp  className="h-5 w-5 text-amber-600"  />} />
      ...
      {works.length === 0 ? (
        <EmptyState
          ...
          title="No development works yet"
          description="Create a work record to get started"
        ...
        <div className="space-y-3">
          {works.map((w) => {
            ...
            const utilPct = w.budget && w.budget_used
              ? Math.round((w.budget_used / w.budget) * 100)
              : 0;
            const progress = w.progress_pct || 0;
            const statusClass = STATUS_COLORS[w.status] || "bg-gray-50 text-gray-600 border-gray-200";
            return (
              <Card key={w.id} className="hover:shadow-md transition-shadow">
                <CardContent className="py-0">
                  <div className="flex flex-col md:flex-row md:items-center gap-4 py-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2 mb-1">
                        ...
                        <span
                          onClick={() => setExpandedMediaIds(prev => prev.includes(w.id) ? prev.filter(id => id !== w.id) : [...prev, w.id])}
                      ...
                      <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500">
                        {w.fundSource && (
                          <span className="flex items-center gap-1">
                            <FileText className="h-3.5 w-3.5" /> Source: <span className="font-medium text-gray-700">{w.fundSource}</span>
                          </span>
                        )}
                        {w.location && (
                          <span className="flex items-center gap-1">
                        ...
                        {w.startDate && (
                          <span className="flex items-center gap-1">
                            <CalendarDays className="h-3.5 w-3.5" /> Started: {formatDate(w.startDate)}
                          </span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-3 md:gap-6 text-center flex-shrink-0">
                      <div>
                        <p className="text-[10px] uppercase tracking-wide text-gray-400 mb-0.5">Budget</p>
                        <p className="text-sm font-bold text-gray-900">{w.budget ? formatCurrency(w.budget) : "—"}</p>
                      </div>
                      <div>
                        <p className="text-[10px] uppercase tracking-wide text-gray-400 mb-0.5">Utilized</p>
                        <p className="text-sm font-bold text-green-600">{w.budget_used ? formatCurrency(w.budget_used) : "—"}</p>
                      </div>
                      <div>
                        <p className="text-[10px] uppercase tracking-wide text-gray-400 mb-0.5">Progress</p>
                    ...
                    <div className="md:w-28 flex-shrink-0">
                      <p className="text-[10px] uppercase tracking-wide text-gray-400 mb-1 text-center">{progress}% completed</p>
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      <Button
                        ...
                        onClick={() => { setConfirmDeleteId(w.id); setConfirmDeleteTitle(w.title || w.id); }}
                      >
                  ...
                  {/* Media Gallery */}
                  {expandedMediaIds.includes(w.id) && (
      ...
      <Dialog open={!!confirmDeleteId} onOpenChange={(o) => { if (!o) { setConfirmDeleteId(null); setConfirmDeleteTitle(""); } }}>
        <DialogContent className="max-w-sm">
          ...
          <div className="py-2">
            ...
            <p className="text-xs text-gray-400 mt-1">This will permanently delete the development work and all linked media records. This action cannot be undone.</p>
          </div>
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={() => { setConfirmDeleteId(null); setConfirmDeleteTitle(""); }}>Cancel</Button>
            <Button variant="destructive" onClick={deleteWork} disabled={deleting}>
